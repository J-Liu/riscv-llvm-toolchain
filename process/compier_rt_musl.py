...
def do_config():
    BUILD_COMPILER_RT_MUSL.mkdir(parents=True, exist_ok=True)
    cmd1 = 'cd ' + BUILD_COMPILER_RT_MUSL.as_posix()
    cmd2 = config_cmd
    cmd = cmd1 + ' && ' + cmd2
    ret = run_shell(cmd)
    return ret
...
def config_compiler_rt_musl():
    if not is_compiler_rt_musl_configured():
        ret = do_config()
        if ret != 0:
            print('compiler-rt for musl config failed')
            exit(1)
        compiler_rt_musl_configured()
...
def build_compiler_rt_musl():
    if not is_compiler_rt_musl_built():
        ret = do_build()
        if ret != 0:
            print('compiler-rt for musl build failed')
            exit(1)
        compiler_rt_musl_built()
...
def install_compiler_rt_musl():
    if not is_compiler_rt_musl_installed():
        ret = do_install()
        if ret != 0:
            print('compiler-rt for musl install failed')
            exit(1)
        compiler_rt_musl_installed()


def process_compiler_rt_musl():
    libc = get_libc_type()
    if libc != '-elf':
        config_compiler_rt_musl()
        build_compiler_rt_musl()
        install_compiler_rt_musl()
